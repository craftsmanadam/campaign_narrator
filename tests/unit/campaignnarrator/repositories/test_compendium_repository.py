...
def test_compendium_repository_loads_rule_index_and_topic_markdown(
    tmp_path: Path,
) -> None:
    """Rules methods should load index and markdown from compendium/rules/."""

    compendium_root = tmp_path / "compendium"
    rules_root = compendium_root / "rules"
    ...
    assert repository.load_rules_context_for_topics(("initiative", "combat")) == (
        "# Initiative\n\nRoll initiative before acting.",
        "# Combat\n\nResolve turns in initiative order.",
    )
    assert (
        repository.load_rules_topic_markdown("source/adjudication/initiative.md")
        == "# Initiative\n\nRoll initiative before acting."
    )


def test_compendium_repository_rejects_rule_paths_outside_rules_root(
    tmp_path: Path,
) -> None:
    """Rule topics must not escape the rules/ subdirectory."""

    compendium_root = tmp_path / "compendium"
    (compendium_root / "rules" / "generated").mkdir(parents=True)
    (tmp_path / "escape.md").write_text("outside")
    repository = CompendiumRepository(compendium_root)

    for path in [Path("/etc/passwd"), "../outside.md", "source/../../escape.md"]:
        try:
            repository.load_rules_topic_markdown(path)
        except ValueError:
            continue
        raise AssertionError


def test_compendium_repository_marks_missing_rules_context(tmp_path: Path) -> None:
